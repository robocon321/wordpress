...
if (isset($_GET['options'])) $params = $params . '&options' . $_GET['options'];
?>
<section>
    <div class="container">
        <form id="form">
            <div class="row my-3">
                <div class="col-12">
                    <div class="search"> <i class="fa fa-search"></i> <input type="text" class="form-control" name="search" placeholder="Bạn muốn tìm kiếm gì?" value="<?php echo (isset($_GET['search']) ? $_GET['search'] : '') ?>"> <button class="btn btn-primary">Tìm kiếm</button> </div>
                </div>
            </div>
            <input type="hidden" id="inputOptions" name="options" value="<?php echo (isset($_GET['options']) ? $_GET['options'] : '') ?>" />
            ...
            <input class="options" type="checkbox" id="posts" value="wp_posts" <?php echo (isset($_GET['options']) ? (strpos($_GET['options'], 'wp_posts') > -1 ? 'checked' : '') : '') ?> onclick="changeOptions()" />
            <label class="mr-4" for="posts"> Posts</label>
            <input class="options" type="checkbox" id="forums" value="my_forums" <?php echo (isset($_GET['options']) ? (strpos($_GET['options'], 'my_forums') > -1 ? 'checked' : '') : '') ?> onclick="changeOptions()" />
            <label class="mr-4" for="forums"> Forums</label>
            <input class="options" type="checkbox" id="services" value="my_services" <?php echo (isset($_GET['options']) ? (strpos($_GET['options'], 'my_services') > -1 ? 'checked' : '') : '')  ?> onclick="changeOptions()" />
