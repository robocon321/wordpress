<?php 
  ...
  class EmployeeModel extends Database {    
    ...
    public function getEmployeeSumary($search = '', $offset = 0, $limit = 10, $sortby = 'name', $isASC = true, $conditions = []) {
      $query = "SELECT ME.id, ME.name `name`, ME.email email, ME.phone phone, ME.province province, ME.status `status`, ME.district district , COUNT(MT.id) total, 
      (
          SELECT COUNT(id) FROM my_tasks WHERE status = 1 AND employee_id = MT.employee_id
      ) success_true
      FROM my_employees ME LEFT JOIN my_tasks MT ON ME.id = MT.employee_id ";
      ...
      $query = $query." GROUP BY MT.employee_id, ME.id ";
      if(isset($sortby)) {
